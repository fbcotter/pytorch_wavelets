...
class xfm{J}{scale}(Function):
    ...
    @staticmethod
    def forward(ctx, input, h0o, h1o, h0a, h0b, h1a, h1b, skip_hps, include_scale, o_dim, ri_dim, downsample):
...
for J in range(1,8):
    ...
    for s in ('scale', ''):
        ...
        if s == '':
            fwd_lo_out = 'Yl'
            bwd_lo_in = 'grad_LoLo'
            downsamplefwd = 'Yl = F.avg_pool2d(Yl, 2)'
            downsamplebwd = 'grad_LoLo = 1/4 * F.interpolate(grad_LoLo, scale_factor=2, mode="nearest")'
        ...
        f.write(xfm.format(
            ...
            level2plusbwd=level2plusbwd,
            downsamplefwd=downsamplefwd,
            downsamplebwd=downsamplebwd,
            J=J))
