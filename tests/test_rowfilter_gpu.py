...
def test_odd_size():
    h = [-1, 2, -1]
    y_op = rowfilter(barbara_t, prep_filt(h, 1).cuda())
...
def test_qshift():
    ...
    x = barbara_t
    y_op = rowfilter(x, prep_filt(h, 1).cuda())
    assert list(y_op.shape)[1:] == bshape_extracol
...
def test_even_size_batch():
    ...
    h = [-1, 1]
    y = rowfilter(zero_t, prep_filt(h, 1).cuda())
...
def test_equal_small_in():
    ...
    y = rowfilter(im_t, prep_filt(h, 1).cuda())
...
def test_equal_numpy_biort1():
    ...
    y = rowfilter(barbara_t, prep_filt(h, 1).cuda())
    np.testing.assert_array_almost_equal(y[0].cpu(), ref, decimal=4)
...
def test_gradients():
    ...
    y_t = rowfilter(im_t, prep_filt(h, 1).cuda())
